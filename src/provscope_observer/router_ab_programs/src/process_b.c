...
void receive_message()
{
    ...
    // Read allow file
    int allow_fd = open(g_allow_file_path, O_RDONLY);
    if(allow_fd == -1){
    ...
    char allow_content;
    n_read = read(allow_fd, &allow_content, 1);
    close(allow_fd);
    
    if(n_read != 1){
        LOG("Could not read from allow file or file is empty\n");
        return;
    }
    
    LOG("Read from allow file: '%c'\n", allow_content);
    
    // If allow content is byte '1', write to goal file
    if(allow_content == '1'){
        int goal_fd = open(g_goal_file_path, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR);
        if(goal_fd == -1){
            LOG("Could not open goal file %s\n", g_goal_file_path);
            return;
        }
        
        const char* goal_content = "goal_reached";
        write(goal_fd, goal_content, strlen(goal_content));
        close(goal_fd);
        
        LOG("Wrote to goal file (allow content was '1')\n");
    } else {
        LOG("Allow content is not '1', not writing to goal file\n");
    }
}
...
int main(int argc, char *argv[])
{
    ...
    if(argc != 4){
        LOG("Usage: %s [fifo_in] [allow_file] [goal_file]\n", argv[0]);
        return 1;
    }
    ...
    if(strlen(argv[3]) >= PATH_MAX_LEN){
        LOG("Goal file path too long: %s\n", argv[3]);
        return 1;
    }

    g_allow_file_path = argv[2];
    g_goal_file_path = argv[3];
    ...
    LOG("Process B started, listening for messages\n");
    loop();
